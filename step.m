...
for rubbish = 1, % calm down everybody, we're stepping only once here

    box.iternum = box.iternum + 1;

    % center for the new quadratic model
    box.quad(:,box.iternum) = box.vertices(box.heap(1)).coor;

    % function value and gradient at the center of the new model
    [box.fmin(box.iternum),box.gmin(:,box.iternum)] = box.f(box.quad(:,box.iternum));

    box.fmin(box.iternum) = box.minmax*box.fmin(box.iternum);
    box.gmin(box.iternum) = box.minmax*box.gmin(box.iternum);

    if (box.fmin(box.iternum) < box.UB)
        box.UB = box.fmin(box.iternum);
        z = box.quad(:,box.iternum);
    end
    ...
    while (stackl > 0)
        ...
        for j = 1:adjnum
            ...
            if (ismember(adjacent, ...
                ...
                qnew = evalq(box.vertices(adjacent).coor, box.quad(:,box.iternum), box.fmin(box.iternum), box.gmin(:,box.iternum), box.gamma);

                if (qnew > box.vertices(adjacent).quad)
    ...
    for j = 1:notboundaryl
        ...
        if (deadisboundaryvertex == 0)
            ...
            box.vertices(box.heap(box.heaplength)).quad = inf;
            ...
            box.vertices(deadboundary).quad = evalq(box.vertices(deadboundary).coor, box.quad(:,box.iternum), box.fmin(box.iternum), box.gmin(:,box.iternum),box.gamma);
    ...
    for j = 1:boundaryl
        ...
        qnewdead = evalq(box.vertices(dead).coor, box.quad(:,box.iternum), box.fmin(box.iternum), box.gmin(:,box.iternum),box.gamma);
        ...
        for l = 1:alivelength
            ...
            qnewalive = evalq(box.vertices(alive).coor, box.quad(:,box.iternum), box.fmin(box.iternum), box.gmin(:,box.iternum),box.gamma);
                    ...
                    %(i) its coordinates
                    %%%%%%%%%%%%%%%%%%%%
                    alpha = (qnewalive - box.vertices(alive).quad)/((qnewalive - box.vertices(alive).quad) - (qnewdead - box.vertices(dead).quad));
                    box.vertices(verticesl).coor = alpha*box.vertices(dead).coor  +  (1-alpha)*box.vertices(alive).coor;

                    %%%%%%%%%%%%%%%%%%%%
                    %(ii) its index
                    %%%%%%%%%%%%%%%%%%%%
                    box.vertices(verticesl).index = intersect(box.vertices(alive).index,box.vertices(dead).index);
                    box.vertices(verticesl).index(dim+1) = box.iternum;

                    %%%%%%%%%%%%%%%%%%%%%%%%%
                    %(iii) its function value
                    %%%%%%%%%%%%%%%%%%%%%%%%%
                    box.vertices(verticesl).quad = evalq(box.vertices(verticesl).coor, box.quad(:,box.iternum), box.fmin(box.iternum), box.gmin(:,box.iternum), box.gamma);
                    for k = 1:box.iternum-1

                        qval = evalq(box.vertices(verticesl).coor, box.quad(:,k), box.fmin(k), box.gmin(:,k), box.gamma);
                        if (qval > box.vertices(verticesl).quad)
                            box.vertices(verticesl).quad = qval;
                        end
                    end
        ...
        if (deadisboundaryvertex == 0)
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            box.vertices(dead).quad = qnewdead;
    ...
    for j = 1:deadboundaryl
        for k = j+1:deadboundaryl
            if (length(intersect(box.vertices(deadboundarylist(j)).index,box.vertices(deadboundarylist(k)).index)) == dim)
